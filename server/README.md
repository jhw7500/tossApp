...
Keep each previous secret for the full period in which a user created under that version may return. Because the original anonymous key is never stored, an identity that has not returned during the rotation window cannot be converted after its previous secret is removed. Removing that secret would cause such a user to be treated as new. Configuration rejects reserved or malformed versions, secrets shorter than 32 characters, reused secrets, an active version repeated in the previous map, and more than eight retained previous versions.
...
Every field must pass for a reader-content `PASS`. If any candidate has
...
and must not be recorded as evidence of tarot-reader content quality. Use
