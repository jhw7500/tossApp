...
declare module 'fastify' { interface FastifyRequest { userId?: string; sessionToken?: string; } }

const noQuerySchema = { type: 'object', additionalProperties: false } as const;

export async function buildApp(options: { config: FoundationConfig; pool: Pool; verifyAnonymousKey?: VerifyAnonymousKey; logger?: FastifyServerOptions['logger'] }): Promise<FastifyInstance> {
  const app = Fastify({ bodyLimit: 16 * 1024, logger: options.logger ?? false, ajv: { customOptions: { removeAdditional: false, allErrors: true } } });
  ...
  app.get('/health/ready', { schema: { querystring: noQuerySchema } }, async (_request, reply) => { try { await options.pool.query('SELECT 1'); return { status: 'ready' }; } catch { return reply.status(503).send({ status: 'unavailable' }); } });
  return app;
}
