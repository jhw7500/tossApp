...
export function ReadingSetupPage({ personId, service, onAccepted, onRecovered }: { personId: string; service: BrowserService; onAccepted: (accepted: AcceptedReading) => void; onRecovered: (recovered: RecoveredReading) => void }) {
  ...
  return <><PageIntro eyebrow={person.nickname} title="무엇을 물어볼까요?" description="질문과 각 위치에 놓을 서로 다른 카드 세 장을 골라 주세요." /><div className="content-stack">
    {error && <div className="error-message" role="alert">{error}</div>}
    {pendingAvailable && <section className="state-card recovery-action" role="status"><h2>먼저 보낸 리딩을 확인해 주세요</h2><p>작성 중인 내용과 별개로, 접수 결과가 불확실한 이전 요청이 남아 있어요.</p><Button display="full" variant="weak" loading={recovering} disabled={recovering} onClick={() => void recoverPending()}>접수 중인 리딩 확인</Button></section>}
    ...
    {selectedQuestion?.isCustomTemplate && <label className="field"><span>직접 질문</span><TextArea variant="box" minHeight={100} value={customText} maxLength={500} placeholder="궁금한 내용을 500자 안으로 적어 주세요." aria-label="직접 질문" onChange={event => setCustomText(event.target.value)} /><small>{Array.from(customText).length}/500</small></label>}
    {selectedQuestion?.positions.map(position => <fieldset className="choice-group" key={position.position}><legend>{position.position}. {position.label}</legend>{position.description && <p className="field-description">{position.description}</p>}<div className="card-grid">{cards.map(card => { const selectedHere = selections[position.position] === card.id; const selectedElsewhere = Object.entries(selections).some(([index, id]) => Number(index) !== position.position && id === card.id); return <button type="button" className={`tarot-card ${selectedHere ? 'tarot-card--selected' : ''}`} aria-pressed={selectedHere} disabled={selectedElsewhere} key={card.id} onClick={() => { setSelections(value => toggleCardSelection(value, position.position, card.id)); setError('') }}><strong>{card.name}</strong><small>{selectedHere ? '다시 눌러 해제' : '카드 선택'}</small></button> })}</div></fieldset>)}
    <Button display="full" loading={submitting} disabled={submitting} onClick={() => void submit()}>리딩 만들기</Button>
  </div></>
}
