...
`restore-check`는 시험 콘텐츠를 넣은 DB를 기준으로 한다. 선택 가능한 카드가 3장 이상이고 각 카드에 활성 한국어 해석 버전이 연결되어 있어야 성공한다. 해석 본문은 실제 카드 조회와 동일하게 유니코드 공백을 제거한 뒤에도 내용이 있어야 한다. 대상 생성에 사용할 수 있는 관계 코드와 연결되고 위치 3개를 갖춘 활성 질문도 1개 이상 필요하다. migration 이력만 있거나 기본 콘텐츠가 빠진 덤프는 실패하며, 아직 Person·Reading이 없는 초기 시험 DB는 허용한다. 이 검사는 복원 가능성·최소 콘텐츠·DB 제약을 확인한다. 백업 시점의 전체 행 수와 대조하는 검사는 아니므로 일부 사용자 데이터 누락까지 판별하지는 않는다.

timer는 호스트 시간대 기준 매일 04:00부터 최대 5분 이내에 실행하며 놓친 실행을 보충한다. `Linger=yes`여야 로그아웃 후에도 user manager가 유지된다. 설치 명령은 이 값을 먼저 확인하고, 꺼져 있거나 확인하지 못하면 unit 파일 작성·timer 활성화 전에 중단한다. 꺼져 있으면 해당 호스트에서 `rtk proxy loginctl enable-linger <사용자 UID>`로 설정한 뒤 설치를 다시 실행한다. 설치 명령이 lingering을 자동 변경하지는 않는다. 현재 PC는 이미 설정되어 있다. 설치된 unit은 현재 checkout의 관리 스크립트를 참조하므로 worktree 이동/삭제 전에 새 경로에서 다시 설치하거나 timer를 해제해야 한다. 사용자 unit의 재부팅 후 실행과 Docker 시작은 실제 호스트 부팅 조건에 의존한다.
...
rtk proxy env TARORORO_RESTORE_INTEGRATION=1 python3 -B -m unittest discover -s deploy/pc-test -p 'test_restore_integration.py'
rtk proxy python3 deploy/pc-test/manage.py status
...
복원 통합 테스트는 실행 중인 PC 테스트 DB 컨테이너가 필요하다. 무작위 이름의 fixture DB와 복원 DB만 만들고 정리하며, 기존 앱 DB·백업은 수정하지 않는다. 기본 단위 테스트 명령에서는 생략된다.
