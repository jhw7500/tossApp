...
이 문서의 초기 검증과 연결 불가 기록은 2026-09-11 시점의 결과다. 2026-09-12에는 mTLS·HTTPS PC 테스트 서버 연결을 완료했고, 토스 번들 `20260912-2`의 인연 등록부터 리딩·기록 재방문까지 사용자가 확인했다. 최신 실기기 검증 근거와 재확인 링크는 [PC 테스트 서버 검증](pc-test-server-verification.md)을 따른다. 아래 초기 연결 불가 상태를 현재의 미해결 문제로 해석하지 않는다.
...
실제 토스 사용자 식별·HTTPS 서버 통신·실기기 리딩 재접속은 당시 로컬 검증에 포함되지 않았다. 이후 검증에는 tarororo용 mTLS 인증서와 HTTPS 서버가 필요했고, 위 2026-09-12 후속 기록에서 그 연결 결과를 확인할 수 있다. 콘솔 OAuth 인증은 이 서버 인증서를 대신하지 않는다.
...
본인 토스 앱에서 위 테스트 번들의 진입을 확인했다. 토스 네이티브 상단 헤더와 `tarororo` WebView는 정상적으로 열렸고, 화면은 의도한 `지금은 서비스를 열 수 없어요` 연결 차단 상태를 표시했다. 이는 기기·아이콘 로딩 실패가 아니라 빌드 시 `VITE_API_BASE_URL`이 없어서 `ai-tarot/src/api/service.ts`가 운영 모드의 API 호출을 시작하지 않은 결과다. 당시 서버 환경도 `APP_ENV=local`, `AUTH_MODE=mock`이고 `TOSS_MTLS_CERT_PATH`와 `TOSS_MTLS_KEY_PATH`가 없음을 값 노출 없이 확인했다. 당시 콘솔의 `tarotaro` 워크스페이스는 `verified=false`, `reviewState=DRAFT`, `licenseType=PERSONAL` 상태였다. 이 초기 확인 뒤 공개 HTTPS 서버·전용 테스트 DB, 인증서, Toss 인증 모드, 서버 주소를 넣은 새 프런트 번들을 준비해 후속 검증을 진행했다.
