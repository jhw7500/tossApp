...
- 후속 독립 검토 두 건에서 중복을 제외한 MEDIUM 보완 2건을 확인했다. 설정/상태 경로의 `..`를 파일 생성 전에 거절하고, 공개 직전 실제 API 컨테이너의 `ALLOWED_ORIGINS`도 승인된 QR Origin과 비교하도록 수정했다. 두 문제의 수정 전 실패와 수정 후 통과를 확인했으며 배포 테스트는 13개로 늘었다. 정상 외부 경로의 준비와 정상 Origin의 공개 호출도 회귀 테스트로 유지했다. 실제 배포 설정 검사를 다시 통과했고 앱·DB 재시작은 필요하지 않았다.
- 3인 사전 검토는 실행 환경의 에이전트 수 제한으로 마지막 역할을 실행하지 못했다. 기존 두 원문 보고서·검증 영수증을 보존하고 전체 사전 검토 통과로 표시하지 않았다. 사용자가 PR 생성을 이 절차로 막는 것에 이의를 제기한 뒤 일반 PR·원격 리뷰로 진행한다. 위 수정 후 커밋은 이전 보고서의 고정 스냅샷과 다르므로 이전 보고서를 새 커밋 전체의 승인으로 사용하지 않는다.
- PR #3의 Codex 리뷰에서 콘텐츠가 없는 덤프도 복원 성공으로 표시하는 문제를 확인했다. 복원된 DB에 선택 가능한 카드 3장 이상과 각 카드의 활성 한국어 해석 버전 연결이 있어야 성공하도록 보완했다. Person·Reading이 없는 초기 시험 DB는 허용한다. 수정 전 실패 3개 하위 사례를 확인했고 수정 후 배포 테스트 15개가 통과했다. 별도 실제 PostgreSQL fixture에서 migration 이력만 있는 덤프와 유효 해석이 연결된 카드가 2장인 덤프의 거절, 사용자 데이터가 없는 정상 카탈로그의 성공을 확인했다. 기존 실제 백업도 Person 1, Reading 1, 유효 카드 3으로 복원에 성공했다. 임시 복원 DB는 모두 정리되어 0개이며 기존 DB·백업은 변경하지 않았다. 이 검사는 최소 콘텐츠와 제약을 확인하며 백업 시점의 전체 행 수와 대조하는 검사는 아니다.
- 후속 Codex 리뷰의 공백 해석 지적도 실제 PostgreSQL 덤프로 재현했다. 빈 문자열·ASCII 공백·ECMAScript 유니코드 공백만 있는 활성 해석을 모두 거절하도록 카드 조회와 동일한 본문 조건을 적용했다. `test_restore_integration.py`는 명시적으로 활성화할 때 실제 임시 DB와 덤프를 생성해 검증한다. 수정 전 공백 사례 3개가 잘못 통과했고, 수정 후 배포 단위 15개와 복원 통합 1개가 모두 통과했다. 공백 사이에 실제 한국어 본문이 있는 초기 카탈로그는 허용하며, 기존 성공 백업 2개도 각각 복원에 성공했다. 검증 후 임시 복원 DB는 0개다.
- 카드 외 기본 콘텐츠에 대한 추가 지적을 반영해, 유효한 관계 코드와 연결되고 위치 3개를 갖춘 활성 질문도 필수로 검사한다. 실제 복원 통합 테스트에서 잘못된 관계 코드·비활성 질문·위치 누락·질문 누락·관계 유형 누락 5개 사례의 수정 전 실패를 확인했다. 수정 후 배포 단위 15개와 복원 통합 1개가 모두 통과했다. 기존 백업 2개도 유효 질문 5개, 읽을 수 있는 카드 3개로 각각 복원에 성공했으며 임시 복원 DB는 0개다.
- 추가 Codex 리뷰의 lingering 지적을 반영했다. 타이머 설치 전에 `loginctl show-user ... -p Linger --value`가 정확히 `yes`인지 확인하고, `no`·빈 값·알 수 없는 값·조회 실패에서는 unit 파일 작성과 timer 활성화 전에 중단한다. 설정을 자동 변경하지 않고 `loginctl enable-linger`와 재실행을 안내한다. 수정 전 잘못된 성공/검사 순서 5개 사례를 재현했고 수정 후 배포 단위 17개가 통과했다. 실제 PC의 `Linger=yes`, 기존 backup timer의 enabled·active 상태를 읽기 전용으로 확인했다. 이 보완에서 기존 unit·타이머·서버를 재설치하거나 재시작하지 않았다.
