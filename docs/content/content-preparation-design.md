...
입력은 UTF-8 CSV 한 파일이다. 엑셀 또는 구글시트에서 편집·내보내기할 수 있도록 선택적인 UTF-8 BOM과 RFC 4180 방식의 인용부호, 쉼표, CRLF/LF, 인용된 필드 안의 줄바꿈을 지원한다. 인용 필드 내부의 LF와 CRLF 시퀀스는 원문 그대로 보존한다. 첫 행은 아래의 정확한 헤더 순서를 사용한다.
