...
복사한 파일을 엑셀이나 구글시트에서 연다. 원문 한 버전을 한 행에 입력하고 `CSV UTF-8` 형식으로 내보낸다. 본문 안의 쉼표, 따옴표, 줄바꿈은 한 셀 안에 유지한다. 검증기는 인용 필드 내부의 LF와 CRLF를 원문 그대로 보존한다. 스프레드시트가 내보낼 때 필요한 CSV 인용부호를 붙이므로 직접 분리하지 않는다.
...
- 가장 큰 `ko-KR` 카드 세 장의 후보 합계가 64 KiB에 도달하거나 넘는 명백한 실패
